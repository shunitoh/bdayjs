...
export default class BusinessDay {
  ...
  addHolidayHours(request: AddHolidayType) {
    ...
    while (days - countDay >= 0) {
      ...
      if (this.hd.isHoliday(currentDay.toDate()) || [0, 6].includes(currentDay.day())) {
  ...
  public subtract = (day, num: number, unit: 'hours' | 'days' = 'hours') =>
    this.dj(day).subtract(this.addHolidayHours({ day, mode: 'subtract', num, unit }), 'hours');
  public add = (day, num: number, unit: 'hours' | 'days' = 'hours') =>
